-- 统一变量存储到 argus_gconfig
-- 执行前请先备份数据库

-- 1) 扩展字段
ALTER TABLE argus_gconfig
...
-- 3) 历史数据兜底：默认按全局变量处理
UPDATE argus_gconfig
SET type = 1
WHERE type IS NULL;

-- 4) 运行时变量查询索引
CREATE INDEX idx_gconfig_runtime_lookup
    ON argus_gconfig(type, env, project_id, case_id, `key`, deleted_at, id);

-- 5) 调整唯一约束
-- 注意：请先确认你库里旧唯一索引/约束的实际名字，再替换后执行 DROP
-- 常见名字可能是: env / argus_gconfig_env_key_deleted_at / uq_...
-- 示例：
-- ALTER TABLE argus_gconfig DROP INDEX env;

ALTER TABLE argus_gconfig
    ADD UNIQUE KEY uq_gconfig_dim(env, `key`, type, project_id, case_id, deleted_at);

-- 6) argus_runtime_variable 已弃用，可按需删除（确认无回滚需求再执行）
-- DROP TABLE argus_runtime_variable;
