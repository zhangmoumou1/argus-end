...
class PityRedisManager(object):
    ...
    @staticmethod
    def _build_client(host: str, port: int, db: int, password: str):
        pool = ConnectionPool(
            host=host,
            port=port,
            db=db,
    ...
    @staticmethod
    def get_writable_client(host: str, port: int, db: int, password: str):
        """
        优先连接可写主库:
        1. 先连配置地址
        ...
        try:
            ...
            logger.bind(name=None).warning(
                f"detect redis writable node failed, use configured node {host}:{port}, error={e}"
            )
        return client
    ...
    def get_cluster(address: str):
        ...
        try:
            nodes = address.split(',')
            startup_nodes = [{"host": n.split(":")[0], "port": int(n.split(":")[1])} for n in nodes if ":" in n]
...
class RedisHelper(object):
    ...
    def _readonly_error(error: Exception):
        return "read only replica" in str(error).lower()
            ...
            if asyncio.iscoroutinefunction(func):
                ...
                async def wrapper(*args, **kwargs):
                    ...
                    try:
                        ...
                        logger.bind(name=None).warning(f"redis get key failed, fallback to db. key={redis_key}, error={e}")
                        return await func(*args, **kwargs)
                    # 获取最新数据
                    ...
                    try:
                        ...
                        if RedisHelper._readonly_error(e):
                            ...
                            try:
                                ...
                                logger.bind(name=None).warning(
                                    f"redis retry set key failed. key={redis_key}, error={re}"
                                )
                        logger.bind(name=None).warning(f"redis set key failed, skip cache write. key={redis_key}, error={e}")
                    return new_data
