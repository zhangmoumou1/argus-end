            ...
            async with async_session() as session:
                ...
                if report is not None and getattr(report, "plan_id", None):
                    ...
                    if plan is not None and getattr(plan, "case_list", None):
                        try:
                            plan_order = [int(x) for x in str(plan.case_list).split(',') if str(x).strip() != '']
                            order_map = {cid: idx for idx, cid in enumerate(plan_order)}
                            test_data.sort(key=lambda x: (order_map.get(x.case_id, 10 ** 9), x.start_at))
                        except Exception:
                            # 保持原有顺序兜底
                            pass
