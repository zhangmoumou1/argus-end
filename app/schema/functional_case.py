...
class FunctionalCaseSkillRequirementItemForm(BaseModel):
    title: Optional[str] = ""
    text: Optional[str] = ""
    images: List[str] = []
    design_links: List[str] = []

    @validator("title", "text", pre=True, always=True)
    def normalize_text(cls, value):
        return str(value or "").strip()
    ...
    @validator("design_links", pre=True, always=True)
    def normalize_design_links(cls, value):
        if not value:
            return []
        if isinstance(value, list):
            return [str(item).strip() for item in value if str(item).strip()]
...
class FunctionalCaseSkillTaskForm(BaseModel):
    ...
    @validator("requirement_items", pre=True, always=True)
    def normalize_requirement_items(cls, value):
        if not value:
            return []
        if isinstance(value, list):
            return value
