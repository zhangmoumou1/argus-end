...
class Scheduler(object):
    ...
    def parse_cron_trigger(cron: str) -> CronTrigger:
        ...
        """
        fields = [x.strip() for x in cron.split() if x.strip()]
        if len(fields) == 5:
            return CronTrigger.from_crontab(" ".join(fields))
        if len(fields) in (6, 7):
            # quartz里的?在apscheduler里可用*等价替换
            ...
            if len(fields) == 7:
                kwargs["year"] = fields[6]
            return CronTrigger(**kwargs)
        raise ValueError(f"Wrong number of fields; got {len(fields)}, expected 5/6/7")
    ...
    def add_test_plan(plan_id, plan_name, cron):
        return Scheduler.scheduler.add_job(func=Executor.run_test_plan, args=(plan_id,),
                                           ...
                                           trigger=Scheduler.parse_cron_trigger(cron))
    ...
    def edit_test_plan(plan_id, plan_name, cron):
        ...
        Scheduler.scheduler.modify_job(job_id=str(plan_id), trigger=Scheduler.parse_cron_trigger(cron), name=plan_name)
