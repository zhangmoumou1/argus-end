...
def call_kimi_generate(form: FunctionalCaseAIGenerateForm, ai_config=None):
    ...
    logger.info(f"functional-case ai provider={provider}, model={model}, base_url={base_url}")
    logger.info(f"functional-case ai request summary provider={provider}, model={model}, summary={json.dumps(request_summary, ensure_ascii=False)}")
    logger.info(f"functional-case ai request payload provider={provider}, model={model}, payload={json.dumps(loggable_payload, ensure_ascii=False)}")
    ...
    try:
        ...
        logger.warning(
            f"functional-case ai timeout provider={provider}, model={model}, elapsed={elapsed}s, summary={json.dumps(request_summary, ensure_ascii=False)}"
        )
        raise ValueError(f"AI模型({provider}/{model})请求超时({elapsed}s)，请减少图片数量或缩小需求内容后重试") from exc
    except requests.RequestException as exc:
        elapsed = round(time.perf_counter() - started_at, 2)
        logger.error(
            f"functional-case ai request failed provider={provider}, model={model}, elapsed={elapsed}s, summary={json.dumps(request_summary, ensure_ascii=False)}, error={exc}"
        )
        raise
    elapsed = round(time.perf_counter() - started_at, 2)
    logger.info(
        f"functional-case ai response provider={provider}, model={model}, status={response.status_code}, elapsed={elapsed}s, body_preview={preview_text(response.text, 1000)}"
    )
    if response.status_code >= 400:
        ...
        raise ValueError(f"AI模型({provider}/{model})调用失败: {detail}")
