...
async def list_project(page: int = 1, size: int = 8, name: str = "", user_info=Depends(Permission())):
    ...
    return ArgusResponse.success_with_size(data=result, total=total)
    ...
    try:
        async with session.begin():
            ...
            if not owner and user_info["role"] != Config.ADMIN:
                return ArgusResponse.forbidden()
            await ProjectDao.delete_record_by_id(session, user_info['id'], projectId, log=True, session_begin=True)
            # 有可能项目没有测试计划 2022-03-14 fixed bug
            await ArgusTestPlanDao.delete_record_by_id(session, user_info['id'], projectId, key="project_id",
                                                      exists=False, log=True, session_begin=True)
        return ArgusResponse.success()
    except Exception as e:
        return ArgusResponse.failed(e)
