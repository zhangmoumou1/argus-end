...
async def register(user: UserDto):
    try:
        ...
        return ArgusResponse.success(dict(token=token, user=user, expire=expire), msg="注册成功, 请登录")
    ...
    try:
        if user['role'] != Config.ADMIN:
            if user['id'] != user_info.id:
                # 既不是改自己的资料，也不是超管
                return ArgusResponse.failed(FORBIDDEN)
            # 如果不是超管，说明是自己改自己，不允许自己改自己的角色
            user_info.role = None
        user = await UserDao.update_user(user_info, user['id'])
        return ArgusResponse.success(user, exclude=("password", "phone", "email"))
...
async def query_user_info(token: str):
    try:
        ...
        if user is None:
            return ArgusResponse.failed("用户不存在")
        return ArgusResponse.success(user, exclude=("password",))
    except Exception as e:
...
async def delete_user(id: int, user=Depends(Permission(Config.ADMIN))):
    ...
    try:
        ...
        return ArgusResponse.success(user)
    except Exception as e:
        return ArgusResponse.failed(e)
...
async def check_reset_url(token: str):
    try:
        ...
        return ArgusResponse.success(email)
    except:
        return ArgusResponse.failed("重置链接不存在, 请不要无脑尝试")
